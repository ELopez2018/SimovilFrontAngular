...
export class StationReceivableComponent implements OnInit {
    ...
    Eliminar(CuentaCobro: EntReceivable, index) {
        Swal.fire({
            title: '¿ESTA SEGURO?',
            text: '¿Desea eliminar la Cuenta de Cobro N° ' + CuentaCobro.num +' ?',
            icon: 'question',
            ...
            if (result.value) {
                this.carteraService
                    .DeleteRecievable(CuentaCobro.id)
                    .subscribe((resp) => {
                        Swal.fire({
                            title: 'CUENTA DE COBRO N° '+CuentaCobro.num+' BORRADA',
                            icon: 'success',
                            text: resp[0].resp,
                        });
                        this.searchReceivable.splice(index, 1);
                    });
            } else {
                return;
            }
