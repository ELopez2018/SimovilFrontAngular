...
export class SheetDailySearchComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.GetEstaciones();
        this.fechaIni = dateToISOString(fechas[0]);
        this.fechaFin = dateToISOString(fechas[1]);
    }
    GetEstaciones() {
    ...
    getDailySheets() {
        ...
        let staionCod = this.stationCode
            ? this.stationCode
            : this.stationSel
        ...
        this.carteraService
            .getDailySheet(
                staionCod,
                this.typeSel ? this.typeSel.id : null,
                this.fechaIni,
                this.fechaFin,
                null,
                false
            ...
            .subscribe(
                ...
                (error) => {
                    this.principal.showMsg('warn', 'ERRORES', error);
                    console.log(error);
                    this.utilService.loader(false);
                }
            );
    ...
    getDailySheetComplete(
        dailySheet: EntDailySheet
    ): Observable<EntDailySheet> {
        this.utilService.loader(true);
        return new Observable<EntDailySheet>((observer) => {
            this.carteraService
                .getDailySheet(null, null, null, null, dailySheet.ID, true)
                .subscribe((data) => {
                    this.utilService.loader(false);
                    ...
                    if (data && data.length == 1) {
                        ...
                        observer.error(
                            'Error al obtener la planilla, multiples o ningun resultado.'
                        );
                    }
                    observer.complete();
                });
    ...
    download(dailySheet: EntDailySheet, view) {
        this.getDailySheetComplete(dailySheet).subscribe(
            (planilla) => {
                //console.log('download=>planilla 1 ', JSON.stringify(planilla));//b de solo el console
    ...
    getAttached(item: EntDailySheetSupport) {
        this.utilService.loader(true);
        this.carteraService.GetPaymentCertificate(item.RUTA).subscribe(
            (result) => {
