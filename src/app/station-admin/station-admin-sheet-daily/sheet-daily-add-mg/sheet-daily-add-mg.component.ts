  ...
  export class SheetDailyAddMGComponent extends ComponentCanDeactivate implements OnInit {
    ...
    initial() {
        ...
        forkJoin(
            ...
            if (res2.length === 1) {
                this.station = res2[0];
                //console.log('ver estación:) '+this.station.idEstacion);//b
    ...
    save(val: boolean) {
        ...
        //console.log('%c OI :) '+JSON.stringify(OI), 'color: green; font-weight: bold;');//b
        ...
        /* if (this.dataDaily.PLA_DIA_PAG_CLI_DET && this.dataDaily.PLA_DIA_PAG_CLI_DET.length >= 0 && this.dataDaily.PLA_DIA_PAG_CLI_DET !== undefined) { */
            /* if (this.dataDaily.PLA_DIA_PAG_CLI_DET && this.dataDaily.PLA_DIA_PAG_CLI_DET.length >= 0) {
       ...
       /*  planilla.OI_PAG_CAR_CLI = car;
        ...
        planilla.OI_PAG_CAR_CLI = OI.oiPagCarCli;
        planilla.OI_PAG_CAR_CLI_DAT = OI.oiPagCarCliDat;
        planilla.OI_PAG_ANT_CLI = OI.oiPagAntCli;
        ...
        //console.log('%c FP: '+JSON.stringify(FP), 'color: blue; fon-weight:bold;');//b
        ...
        if (this.dataDaily.PLA_DIA_VEN_CLI && this.dataDaily.PLA_DIA_VEN_CLI.length > 0) {
            this.dataDaily.PLA_DIA_VEN_CLI_DET.map(e => {
                console.log("test1, cod: "+e.COD_CLIENTE+" tipo: "+e.TIPO_CLIENTE)
        ...
        planilla.FP_CLI_CRE = FP.ventaCliCredito;
        planilla.FP_CLI_ANT = FP.ventaCliAnticipo;
        ...
        //console.log('%c planilla:) '+JSON.stringify(planilla), 'color: red; font-weight: bold;');//b
        this.boolSave = true;
    }
    ...
    buildForms() {
        this.otherForm = this.fb.group({
            ...
            oiPagCarCli: [0, [Validators.required, Validators.min(0)]],
            oiPagCarCliDat: [0, [Validators.required, Validators.min(0)]],
            oiPagAntCli: [0, [Validators.required, Validators.min(0)]],
        ...
        this.paymentForm = this.fb.group({
            ...
            /****************************************************************/
            ventaCliCredito: [0, [Validators.required, Validators.min(0)]],
            ventaCliAnticipo: [0, [Validators.required, Validators.min(0)]],
            /****************************************************************/
        ...
        this.otherForm.valueChanges.subscribe(e => {
            setTimeout(() => {
                ...
                this.otherForm.get('total').setValue(a.aprovVal + a.oiPagCarCli + a.oiPagCarCliDat + a.oiPagAntCli + a.lubricante + a.otroVal + a.premio + a.prestamoVal + a.soatCom + a.soatValue + a.presLiq + a.cusiana, this.emitFalse);
            }, 10);
        });
        this.paymentForm.valueChanges.subscribe(e => {
            setTimeout(() => {
                ...
                this.paymentForm.get('total').setValue((a.bonoSoat || 0) + (a.calibracion || 0) + (a.ventaCliCredito || 0) + (a.ventaCliAnticipo || 0) + (a.datafono || 0) + (a.descuento || 0) + (a.devolucion || 0) + (a.donacion || 0) + (a.mantenimiento || 0) + (a.prestamo || 0) + (a.otro || 0) + (a.bonoCumple || 0) + (a.bonoPunto || 0), this.emitFalse);
            }, 10);
        });
    ...
    estaAutorizada(){
        if(this.estacionId == this.gasollanos){
            console.log('sì es gasollanos');
            return false;
        }
        if(this.estacionId == this.pavitos){
            console.log('sì es pavitos');
            return false;
        }
        if(this.estacionId == this.cusiana){
            console.log('sì es cusiana');
            return false;
        }
