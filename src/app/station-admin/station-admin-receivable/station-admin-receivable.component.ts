...
export class StationAdminReceivableComponent implements OnInit {
    ...
    autoCreate() {
        ...
        this.utilService.confirm(
            'Generará las cuentas de cobro de los clientes asignados a su estación. ¿Desea continuar?',
            (res) => {
                if (res) {
                    const fech = dateToISOString(addDays(this.fecha, 1));
                    this.utilService.loader();
                    this.carteraService
                        .createReceivable(fech, this.codStation)
                        .subscribe(
                            (res) => {
    ...
    getclientsPending() {
        this.utilService.loader();
        this.carteraService.getReceivablePending(this.codStation).subscribe(
            (r) => {
                this.utilService.loader(false);
                this.clients = r;
            },
    ...
    printRec(id) {
        this.utilService.confirm(
            '¿Desea imprimir la cuenta de cobro?',
            (res) => {
                if (res) {
                    this.utilService.loader();
                    forkJoin(
                        this.carteraService.getReceivable(
                            null,
                            null,
                            null,
                            null,
                        ...
                        this.carteraService.getConsumption(
                            null,
                            null,
                            null,
                            id,
