...
export class BsSearchComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.carteraService.getStationType().subscribe(
            (res) => {
    ...
    getStation() {
        this.utilService.loader(true);
        let buscar = '';
        if (this.searchid.trim() == '') buscar = null;
        else buscar = this.searchid.trim().replace(' ', '%');
        this.nominaService.GetStations(null, buscar).subscribe(
            (result) => {
                this.utilService.loader(false);
    ...
    getType(station: EntStation) {
        try {
            return this.stationType.find(
                (e) => e.idTipoEstacion == station.tipoEstacion
            ).detalleTipoEstacion;
        } catch (error) {
            return;
        }
    ...
    deleteStation(station: EntStation) {
        this.utilService.confirm(
            '¿Desea eliminar la estación ' + station.nombreEstacion + '?',
            (data) => {
                if (data) {
                    this.nominaService.DeleteStation(station).subscribe(
                        (result) => {
                            ...
                            this.stations = this.stations.filter(
                                (e) => e.idEstacion != station.idEstacion
                            );
                        },
                        (error) => {
                            this.principal.showMsg(
    ...
    resEdit(res: { obj: EntStation; result: boolean }) {
        if (res.result) {
            let index = this.stations.findIndex(
                (e) => e.idEstacion == res.obj.idEstacion
            );
            ...
            .subscribe(
                (res) => {
                    this.principal.showMsg(
                        'success',
                        'Éxito',
                        'Edición ' + (val ? 'habilitada' : 'deshabilitada')
                    );
                },
                (error) => {
                    this.principal.showMsg(
